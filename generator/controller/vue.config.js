 ...
 * @LastEditTime: 2021-08-06 17:33:54
 ...
 * @return {*}
 */
const vueConfigAddPx2rem = (api) => {
  api.afterInvoke(() => {
    const fs = require('fs')
    const { EOL } = require('os')
    const contentVueConfig = fs.readFileSync(api.resolve('./vue.config.js'), {
      encoding: 'utf-8'
    })
...
const vueConfigAddFlexible = (api) => {
  const fs = require('fs')
  try {
...
module.exports = (api) => {
  return {
    vueConfigAddPx2rem: vueConfigAddPx2rem(api),
    vueConfigAddFlexible: vueConfigAddFlexible(api)
  }
}
