  ...
  return {
    ...
    addFlexibleOptions() {
      ...
      api.afterInvoke(() => {
        ...
        const contentVueConfig = fs.readFileSync(api.resolve(api.entryFile), { encoding: 'utf-8' })
        const configLines = contentVueConfig.split(/\r?\n/g)
        const configIndex = configLines.findIndex(line => line.match(/lib-flexible/))
        if (configLines.findIndex(line => line.match(/rem.js/)) === -1) {
          configLines[configIndex] += `${EOL} import './utils/rem.js'`
          fs.writeFileSync(api.entryFile, configLines.join(EOL), { encoding: 'utf-8' })
        }
      })
    },
    /**
